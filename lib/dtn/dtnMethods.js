...
const handleAAPForwarder = async (type, modelName, fileHistory, data) => {
  await AAPForwarder({
    message: {
      type,
      modelName,
    ...
    eids: delayTolerantMongoose.dtnConfig.EID_LIST, // TODO: Get the EID list from somewhere else.
    dtnHost: delayTolerantMongoose.dtnConfig.DTN_HOST, // Same as above.
    dtnPort: delayTolerantMongoose.dtnConfig.DTN_PORT, // Same as above.
  });
}
 ...
 */
const handleCreationMethods = async (actionType, localParams) => {
...
const handleDeletionMethods = async (actionType, localParams) => {
  ...
  if (["deleteOne", "findOneAndDelete", "findByIdAndDelete", "findOneAndRemove", "findByIdAndRemove"].includes(actionType)) {
  ...
  } else if (actionType === "deleteMany" || actionType === "findManyAndDelete") {
    ...
    documents.forEach(document => handleFileHistory(document, actionType));
...
const handleUpdateMethods = async (actionType, localParams) => {
  ...
  if (["updateOne", "replaceOne", "findOneAndReplace", "findOneAndUpdate", "findByIdAndUpdate"].includes(actionType)) {
