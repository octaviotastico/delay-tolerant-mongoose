...
export const pushTohistory = async ({ document, changeID, fileHistoryID }) => {
  await FileHistoryModel.updateOne(
    fileHistoryID ? { _id: fileHistoryID } : { documentID: document._id },
  ...
  return await FileHistoryModel.findOne(fileHistoryID ? { _id: fileHistoryID } : { documentID: document._id },);
};
 ...
 * @param {Object} filter1: Filter to get the second local file history object.
 */
