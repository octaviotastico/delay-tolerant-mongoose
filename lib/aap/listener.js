...
const threeWayMergeStrategy = ({ model, data, method,   }) => {
  return;
};

const updateLocalDatabase = async ({ type, modelName, localDate, fileHistory, data }) => {
  ...
  if (type === "create") {
    ...
    if (mergeStrategy === "none") {
      noneMergeStrategy({ model, data, method: "updateOne", localDate });
    ...
    if (mergeStrategy === "none") {
      noneMergeStrategy({ model, data, method: "findOneAndUpdate", localDate });
    } else if (mergeStrategy === "threeWayMerge") {
      return;
    }
    ...
    if (mergeStrategy === "none") {
      noneMergeStrategy({ model, data, method: "findByIdAndUpdate", localDate });
    } else if (mergeStrategy === "threeWayMerge") {
      return;
    }
