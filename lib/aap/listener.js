...
// Local imports
import { saveNewHistory, createNewHistory, deleteHistory, pushTohistory } from "../fileHistory/fileHistoryActions";
import { isCreationMethod, isUpdateMethod, isDeletionMethod } from "../dtn/commons";
 ...
 * @param {object} data - Payload to be sent to the mongoose method.
...
const updateLocalDatabase = async ({ actionType, modelName, localDate, fileHistory, data }) => {
  ...
  if (actionType === "create") {
    if (mergeStrategy === "none") {
      model.create(data);
    } else if (mergeStrategy === "threeWayMerge") {
      threeWayMergeStrategy({ model, data, method: actionType, fileHistory });
    }
  } else if (actionType === "insertMany") {
    if (mergeStrategy === "none") {
      model.insertMany(data);
    } else if (mergeStrategy === "threeWayMerge") {
      threeWayMergeStrategy({ model, data, method: actionType, fileHistory });
    }
  } else if (actionType === "deleteOne") {
    model.deleteOne(data).exec();
  } else if (actionType === "deleteMany") {
    model.deleteMany(data).exec();
  } else if (actionType === "updateOne") {
    if (mergeStrategy === "none") {
    ...
    console.log(`[ERROR] Unknown operation actionType: ${actionType}`);
  }
};
