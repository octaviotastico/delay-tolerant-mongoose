...
const threeMergeStrategyUpdate = async ({ model, data, method, fileHistory, latestLocalVersion }) => {
  ...
  const latestLocalVersionChangeID = await getLatestChangeID({ documentID: latestLocalVersion._id });
  ...
  const parentVersion = await getLatestCommonParent({ documentID: latestLocalVersion._id }, fileHistory);
