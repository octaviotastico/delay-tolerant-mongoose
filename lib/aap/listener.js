...
// Local imports
import { saveNewHistory, deleteHistory, pushTohistory, getLatestChangeID } from "../fileHistory/fileHistoryActions.js";
...
const threeMergeStrategyUpdate = async ({ model, data, method, fileHistory, latestLocalVersion }) => {
  ...
  const latestLocalVersionChangeID = getLatestChangeID({ documentID: latestLocalVersion._id });
  ...
  const parentVersion = getLatestCommonParent({ documentID: latestLocalVersion._id }, fileHistory);
...
const threeWayMergeStrategy = async ({ model, data, method, fileHistory }) => {
  ...
  if (isMultipleUpdateMethod(method)) {
    await model.find(filter, (err, docs) => {
      if (err) {
        console.log(`[ERROR] ${method} - Failed to find documents: ${err}`);
        return;
      }

      await Promise.all(docs.map((latestLocalVersion) => {
...
const updateLocalDatabase = async ({ actionType, modelName, localDate, fileHistory, data }) => {
  ...
  switch (mergeStrategy) {
    ...
    case "threeWay":
      await threeWayMergeStrategy({ model, data, method: actionType, fileHistory });
      break;
