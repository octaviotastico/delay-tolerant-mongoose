...
const connectAndRetry = async (dtnHost, dtnPort) => connect(dtnHost, dtnPort)
  .then(connection => connection)
  .catch(async () => {
    console.log('[ERROR] Failed to connect to DTN node, retrying in 5 seconds.');
    await delay(5000);
    return connectAndRetry();
  });

const initializeDTN = async (dtnHost, dtnPort, agentID) => {
  let netClient = await connectAndRetry(dtnHost, dtnPort);
  ...
  netClient.on('end', () => {
    console.log('[INFO] DTN Node disconnected, attempting to recomnnect.');
    global.dtnStatistics.isConnected = false;
    initializeDTN(dtnHost, dtnPort, agentID);
  });
