...
const connectAndRetry = async (dtnHost, dtnPort, agentID, msDelay = 5000) =>
  createClientConnection(dtnHost, dtnPort)
    ...
    .catch(async () => {
      console.log(
        `[ERROR] Failed to connect to μD3TN, retrying in ${
          msDelay / 1000
        } seconds.`
      );
      await delay(msDelay);
...
const initializeDTN = async (dtnHost, dtnPort, agentID) => {
  ...
  netClient.on("end", async () => {
    netClientConnectionEnd(
      netClient,
      "[INFO] μD3TN disconnected, attempting to reconnect."
    );
  });
...
const listen = async (netClient) => {
  ...
  netClient.on("data", async (receivedData) => {
    ...
    global.dtnStatistics.totalBundlesReceived +=
      Buffer.byteLength(Buffer.from(receivedData)) / 1000 / 1000;
    ...
    const deserializedPayload =
      deserializedMessage?.payload?.toString("utf8") || "";
...
const updateLocalDatabase = async ({ type, modelName, localDate, data }) => {
  ...
  if (type === "create") {
    ...
    model.findOne(filter, (err, doc) => {
      ...
      const isValidDate =
        localUpdatedAt instanceof Date && !isNaN(localUpdatedAt);
