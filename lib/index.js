...
const FileHistoryModel = mongoose.model(
  "FileHistory",
  mongoose.Schema({
...
const handleFileHistory = (document, actionType) => {
  if (creationMethods.includes(actionType)) {
    ...
    pushTohistory(document._fileHistoryID, document);
  }
};
...
const createDTNMethod = function (actionType) {
  return async function (localParams) {
    ...
    if (deletionMethods.includes(actionType)) {
      deletedDocument = await this.findOne(localParams);
    }
    ...
    if (creationMethods.includes(actionType)) {
      ...
      // the document before deleting it in the deletedDocument variable.

      if (Array.isArray(deletedDocument)) {
        // If it is an array, we need to handle file history for each one.
        deletedDocument.forEach((document) => {
          handleFileHistory(document, actionType);
        });
      } else {
        // If it is a document, we only need to handle file history for it.
        handleFileHistory(deletedDocument, actionType);
      }
    } else if (updateMethods.includes(actionType)) {
