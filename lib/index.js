...
// Local Imports
import { initializeDTN } from "./aap/listener";
import { createDTNMethod } from "./dtn/dtnMethods";
...
delayTolerantMongoose.availableMergeStrategies = [
  {
    name: "None",
    description: "[Default] No merge strategy is used. Documents are overwritten, depending on the 'send' timestamp (newest wins).",
  },
  {
    name: "ThreeWayMerge",
    description: "Documents are merged in a three way merge. This method uses more memory, but it has the advantage of being able to handle multiple versions of the same document so it does not lose any information (there are no overwrites).",
  }
];
